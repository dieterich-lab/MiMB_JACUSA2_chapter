...
snakemake <- Snakemake(
    ...
    output = list('output/snakemake/analysis/tetR_metvstetR/NMFestimateRank_CophCorr.pdf'),
    params = list(),
    wildcards = list(),
    threads = 1,
    log = list(),
...
estimateRank <- function(data, nb_permutation = 10, interval = seq(2,9), method='snmf/r', nrun=30) {
  cc= matrix(0,nb_permutation,length(interval))
    ...
    { nmf_random= nmf(randomize(data), interval, nrun=nrun,.options='vp30', seed=42)
