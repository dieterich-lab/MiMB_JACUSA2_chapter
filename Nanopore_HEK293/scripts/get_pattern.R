...
estim.r.random <-
  nmf(V.random,
      ...
      .opt = 'vp3')
DeltaSil <-
  estim.r$measures$silhouette.consensus - estim.r.random$measures$silhouette.consensus
DeltaCoph <-
  estim.r$measures$cophenetic - estim.r.random$measures$cophenetic
ChoseRank <-
  min(which(DeltaSil == max(DeltaSil)) + 1, which(DeltaCoph == max(DeltaCoph)) +
        1)
...
barplot(
  ...
  ylim = range(pretty(c(0, DeltaSil))),
  xlab = "Rank",
  ylab = "Delta",
...
abline(h = c(max(DeltaSil) , max(DeltaCoph)) ,
       col = c("#009E73", "#D55E00"))
dev.off()
pdf(args[3])
