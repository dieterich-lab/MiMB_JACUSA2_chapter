...
AllSitesScores <-
  data.frame(AllSitesScores[rownames(BigTable), ], BigTable[, c("Motif", "DRACH")])
...
Noverlap.df <-
  data.frame(
    ID = paste0(Noverlap[, 1], ":", Noverlap[, 2] - 2, "_", Noverlap[, 3] +
                  2, ":", Noverlap[, 6]),
    Type = Noverlap[, 4]
  )
...
ins2 <-
  intersect(
    rownames(AllSitesScores),
    subset(Noverlap.df$ID, Noverlap.df$Type != snakemake@params[[3]])
  )
...
TestSitesScores <-
  data.frame(str_split_fixed(rownames(TestSitesScores), "[:_]", 4), TestSitesScores)
colnames(TestSitesScores)[1:4] <- c('chr', 'start', 'end', 'strand')

#add CLIP label to the scores
dataGG <-
  data.frame(AllSitesScores[c(ins, NoCLIPsites), -c(ncol(AllSitesScores) -
                                                      1, ncol(AllSitesScores))], CLIP = c(as.character(Noverlap.df[ins, 2]), rep("NoCLIP", length(NoCLIPsites))))
...
for (dude in colnames(dataGG)[1:NumberOfFactors])
{
  xdensity <- ggplot(dataGG, aes_string(dude, color = "CLIP")) +
    stat_ecdf() +
    scale_color_manual(values = cbbPalette) + theme_bw() +  scale_x_continuous(name = "score") +
    theme(
      axis.title.x = element_text(size = rel(2)),
      axis.title.y = element_text(size = rel(2)),
      axis.text = element_text(size = rel(1.5))
    )
  ggsave(paste0(snakemake@output[[1]], "/", dude, "_ecdf.pdf"), device =
           "pdf")
}

# ##############################################plot PPV ############################################
x <-
  data.frame(TestSitesScores[, -c(1, 2, 3, 4, ncol(TestSitesScores) - 1, ncol(TestSitesScores))], CLIP =
               c(rep(1, length(ins2)), rep(0, length(NoCLIPsites))))
...
for (dude in colnames(x)[1:NumberOfFactors])
{
  ...
  coordinates <-
    coords(
      ...
      x = "all",
      input = "threshold",
      ret = c("threshold", "ppv", "tpr", "tp", "tn", "fp", "fn")
    )
  ...
  ggplot(coordinates, aes(x = threshold)) +
    ...
    scale_y_continuous(# Features of the first axis
      ...
      sec.axis = sec_axis( ~ . * 5, name = "Positive Predictive Value")) +  scale_x_continuous(name = "Cutoff") +  theme_bw() + theme(
        axis.title.x = element_text(size = rel(2)),
  ...
  ggsave(paste0(snakemake@output[[1]], "/", dude, "_ppv.pdf"), device =
           "pdf")
  scores <-
    TestSitesScores[, c('chr', 'start', 'end', 'Motif', dude, 'strand')]
  colnames(scores)[4:5] <- c('name', 'score')
  write.table(
    scores,
    paste0(snakemake@output[[1]], "/", dude, "_prediction.bed"),
    row.names = FALSE,
    quote = FALSE,
    sep = "\t"
  )
