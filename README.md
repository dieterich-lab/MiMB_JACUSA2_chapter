...
We provide a Snakemake pipeline for RNA modification detection using nanopore Direct RNA-seq of multiple conditions and replicates.
...
# Usage
The following protocol describes how to detect m6A modification from nanopore direct RNA-seq data. The benchmark is obtained from [PRJEB40872](https://www.ebi.ac.uk/ena/browser/view/PRJEB40872?show=reads) and is composed of two samples from two conditions: wild-type cells (WT, modified RNAs) and Mettl3 knockout cells (KO, unmodified RNAs) with two replicates (2 and 3). The supervised analysis (pattern learning) and the evaluation are based on m6A sites from `miCLIP_union.bed` file. The sites are reported in three miCLIP-based studies Bouliaset al. [2019], Koh et al. [2019], Körtel et al. [2021]. The consensus of the three studies annotated with "Boulias,Koertel,Koh" is used for the training process whereas the test set involves the rest of miCLIP sites and non miCLIP sites. [Following the use case 1 of the manuscript, we limited the analysis to the set of sites reported in 'data/selected_regions.bed' file, which refers to the intersection of samples of use cases 1 and 2 ]
...
*   **`prediction/`:**
    *   `pattern{pattern number}_prediction.bed` - estimated scores from selected patterns
